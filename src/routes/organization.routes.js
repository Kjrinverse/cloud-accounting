...
const createOrganizationSchema = Joi.object({
  ...
  fiscalYearStart: Joi.string().pattern(/^\d{2}-\d{2}$/).default('01-01'),
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const userOrgs = await db('user_organizations')
      .join('organizations', 'user_organizations.organization_id', 'organizations.id')
      .where('user_organizations.user_id', req.userId)
      .select(
        'organizations.id',
        'organizations.name',
        'organizations.tax_id as taxId',
        'organizations.address',
        'organizations.phone',
        'organizations.email',
        'organizations.website',
        'organizations.timezone',
        'organizations.fiscal_year_start as fiscalYearStart',
        'organizations.base_currency as baseCurrency',
...
router.get('/:id/users', async (req, res, next) => {
  try {
    ...
    // Check if user has access to this organization
    const hasAccess = await db('user_organizations')
      .where({ 
        user_id: req.userId, 
        organization_id: organizationId 
      })
